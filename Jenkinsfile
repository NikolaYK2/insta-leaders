...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3500
        NAMESPACE = "sociable-people-com"
        REGISTRY_HOSTNAME = "nikolajkevlich"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "inctagram-frontend-main"
        DEPLOYMENT_NAME = "inctagram-frontend-main-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'sociable-people-com') {
