...
const routs: Routs[] = [
  ...
  {
    href: ROUTES_APP.SEARCH,
    label: "Search",
    style: "mb-[38.66%]",
    icon: "Search",
  },
  {
    href: ROUTES_APP.STATISTICS,
    label: "Statistics",
    icon: "TrendingUpOutline",
  },
  {
    href: ROUTES_APP.FAVORITES,
    label: "Favorites",
...
export const Sidebar: NextPageWithLayout = () => {
  ...
  return (
    <nav className="max-w-[156px] min-h-screen mr-[24px] w-full text-white border-r-[1px] border-dark-300 notePad:hidden">
      <section
        className={
          "sticky top-[60px] pt-[74px] flex flex-col max-h-full justify-between mr-4"
        }
      >
        {routs.map((rout) => (
          <Link
          ...
          >
            ...
            {rout.label}
            {rout.label === "Create" && (
              <Create open={open} onOpenChange={setOpen} />
            )}
          </Link>
        ))}
        <LogOut />
