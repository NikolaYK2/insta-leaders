...
const CroppingPhotos = () => {
  ...
  const handleDeletePhoto = async (id: string) => {
    const imageToDelete = images.find((img) => img.uploadId === id);
    // const imageToDelete = images.find(img => img.id === id)
    if (imageToDelete) {
      URL.revokeObjectURL(imageToDelete.url); // Освобождаем память для удалённого URL
  ...
  const handleClickImage = (id: string) => {
    const newIndex = images.findIndex((selectImg) => selectImg.uploadId === id); // Находим индекс выбранного изображения
  ...
  return (
    <div
      className={
        "absolute flex items-start bottom-[60px] right-3 bg-dark-500/40"
      }
    ...
    >
      <div className={"flex flex-wrap overflow-hidden"}>
        {images.map((img) => (
          <div
            key={img.uploadId}
            className="flex max-w-[82px] max-h-[82px] overflow-hidden m-1.5"
          >
            ...
            <PhotoPreview
              ...
              onClick={() => handleClickImage(img.uploadId)}
              callback={() => handleDeletePhoto(img.uploadId)}
      ...
      {/*BUTTON ADD PHOTOS*/}
      <Button
        className={"p-0 m-1.5 text-light-100"}
        variant={"text"}
        onClick={handleClick}
      >
