  ...
  const handleGenerateCroppedImage = async () => {
    ...
    try {
      ...
      if (blob) {
        ...
        await indexDBUtils.updateImageById({
          uploadId: images[indexCropImage].uploadId,
          url: blob,
        });
  ...
  return (
    <>
      <div onDoubleClick={handleGenerateCroppedImage} onTouchEnd={handleTouch}>
        <Cropper
          image={images[indexCropImage]?.url ?? ""} // Показываем текущее изображение
