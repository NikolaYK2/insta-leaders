...
export const GeneralInformation: NextPageWithLayout = () => {
  ...
  const [changeProfile, { data, isError, isLoading }] =
    useUpdateProfileMutation();
  // Получение выбранной даты с помощью кастомного хука
  const { selectedDate } = useSelectedCalendar();

  const { handleSubmit, control, getValues, watch, reset, setValue } =
    useForm<FormType>({
      defaultValues: {
        userName: "",
        firstName: "",
  ...
  useSaveForm({
    ...
    saveValue: (value) => {
      ...
      if (updateValue.dateOfBirth) {
        updateValue.dateOfBirth = new Date(
  ...
  return (
    <Page
      titleMeta={"General information"}
      descriptionMeta={"info"}
      className={"pt-0"}
    >
      <div className={"flex justify-between flex-wrap"}>
        ...
        <form
          className={"flex flex-col max-w-[740px] w-full mt-[36px]"}
          onSubmit={onSubmit}
        >
